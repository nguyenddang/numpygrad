...
def test_attention():
    ...
    blockn.c_attn.weight = npg.Tensor(blockt.c_attn.weight.detach().numpy().T, requires_grad=True)
    blockn.c_attn.bias = npg.Tensor(blockt.c_attn.bias.detach().numpy(), requires_grad=True)
    blockn.c_proj.weight = npg.Tensor(blockt.c_proj.weight.detach().numpy().T, requires_grad=True)
    ...
    assert torch.allclose(torch.from_numpy(blockn.c_attn.weight.grad), blockt.c_attn.weight.grad.T, rtol=1e-5, atol=1e-6)
    assert torch.allclose(torch.from_numpy(blockn.c_attn.bias.grad), blockt.c_attn.bias.grad, rtol=1e-5, atol=1e-6)
    assert torch.allclose(torch.from_numpy(blockn.c_proj.weight.grad), blockt.c_proj.weight.grad.T, rtol=1e-5, atol=1e-6)
...
def test_layernorm():
    ...
    mlpn.c_fc.weight = npg.Tensor(mlpt.c_fc.weight.detach().numpy().T, requires_grad=True)
    mlpn.c_fc.bias = npg.Tensor(mlpt.c_fc.bias.detach().numpy(), requires_grad=True)
    mlpn.c_proj.weight = npg.Tensor(mlpt.c_proj.weight.detach().numpy().T, requires_grad=True)
...
def test_mlp():
    ...
    assert torch.allclose(torch.from_numpy(mlpn.c_fc.weight.grad), mlpt.c_fc.weight.grad.T, rtol=1e-5, atol=1e-6)
    assert torch.allclose(torch.from_numpy(mlpn.c_fc.bias.grad), mlpt.c_fc.bias.grad, rtol=1e-5, atol=1e-6)
    assert torch.allclose(torch.from_numpy(mlpn.c_proj.weight.grad), mlpt.c_proj.weight.grad.T, rtol=1e-5, atol=1e-6)
...
def test_block_manual():
    ...
    attnn.c_attn.weight = npg.Tensor(attnt.c_attn.weight.detach().numpy().T, requires_grad=True)
    attnn.c_attn.bias = npg.Tensor(attnt.c_attn.bias.detach().numpy(), requires_grad=True)
    attnn.c_proj.weight = npg.Tensor(attnt.c_proj.weight.detach().numpy().T, requires_grad=True)
    ...
    assert torch.allclose(torch.from_numpy(attnn.c_attn.weight.grad), attnt.c_attn.weight.grad.T, rtol=1e-5, atol=1e-6)
    assert torch.allclose(torch.from_numpy(attnn.c_attn.bias.grad), attnt.c_attn.bias.grad, rtol=1e-5, atol=1e-6)
    assert torch.allclose(torch.from_numpy(attnn.c_proj.weight.grad), attnt.c_proj.weight.grad.T, rtol=1e-5, atol=1e-6)
...
class test_block():
    ...
    blockn.attn.c_attn.weight = npg.Tensor(blockt.attn.c_attn.weight.detach().numpy().T, requires_grad=True)
    blockn.attn.c_attn.bias = npg.Tensor(blockt.attn.c_attn.bias.detach().numpy(), requires_grad=True)
    blockn.attn.c_proj.weight = npg.Tensor(blockt.attn.c_proj.weight.detach().numpy().T, requires_grad=True)
    ...
    blockn.mlp.c_fc.weight = npg.Tensor(blockt.mlp.c_fc.weight.detach().numpy().T, requires_grad=True)
    blockn.mlp.c_fc.bias = npg.Tensor(blockt.mlp.c_fc.bias.detach().numpy(), requires_grad=True)
    blockn.mlp.c_proj.weight = npg.Tensor(blockt.mlp.c_proj.weight.detach().numpy().T, requires_grad=True)
    ...
    assert torch.allclose(torch.from_numpy(blockn.attn.c_attn.weight.grad), blockt.attn.c_attn.weight.grad.T, rtol=1e-5, atol=1e-6)
    assert torch.allclose(torch.from_numpy(blockn.attn.c_attn.bias.grad), blockt.attn.c_attn.bias.grad, rtol=1e-5, atol=1e-6)
    assert torch.allclose(torch.from_numpy(blockn.attn.c_proj.weight.grad), blockt.attn.c_proj.weight.grad.T, rtol=1e-5, atol=1e-6)
    ...
    assert torch.allclose(torch.from_numpy(blockn.mlp.c_fc.weight.grad), blockt.mlp.c_fc.weight.grad.T, rtol=1e-5, atol=1e-6)
    assert torch.allclose(torch.from_numpy(blockn.mlp.c_fc.bias.grad), blockt.mlp.c_fc.bias.grad, rtol=1e-5, atol=1e-6)
    assert torch.allclose(torch.from_numpy(blockn.mlp.c_proj.weight.grad), blockt.mlp.c_proj.weight.grad.T, rtol=1e-5, atol=1e-6)
